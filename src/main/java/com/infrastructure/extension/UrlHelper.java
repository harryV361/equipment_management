package com.infrastructure.extension;
...
public class UrlHelper {
	...
	public static String resolveWithReturnUrl(String url, Object returnUrl,
			PageContext pageContext) throws JspException,
			UnsupportedEncodingException {
		StringBuilder urlBuilder = new StringBuilder(UrlSupport.resolveUrl(url,
				null, pageContext));

		if (returnUrl != null && !returnUrl.toString().isEmpty())
			urlBuilder.append("?returnUrl=").append(
	...
	public static String resolveWithReturnUrl(String url, Object returnUrl,
			Object returnQuery, PageContext pageContext) throws JspException,
			UnsupportedEncodingException {
		StringBuilder urlBuilder = new StringBuilder(UrlSupport.resolveUrl(url,
				null, pageContext));
	...
	public static String resolveWithReturnUrl(String url, Object returnUrl,
			Object returnQuery, String appendReturnQuery,
			PageContext pageContext) throws JspException,
			UnsupportedEncodingException {
		StringBuilder urlBuilder = new StringBuilder(UrlSupport.resolveUrl(url,
				null, pageContext));
					...
					URLEncoder.encode(
							returnUrl.toString()
									+ resolveQuery(
											returnQuery != null ? returnQuery
													.toString() : null,
											appendReturnQuery), "UTF-8"));

		return urlBuilder.toString();
	}

	private static String resolveQuery(String returnQuery,
			String appendReturnQuery) {
